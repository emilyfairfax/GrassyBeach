% Grasses on a Sandy Beach: Neutral Relationship with Burial
...
% In this particular version of the model, the plant has a neutral
% relationship with burial. This means that the growth rate of the plant is
% based only on the following general equation: G = gB(1-B), where G is the
...
% growth height. There is no "growth factor" term to account for how much
% of the plant is buried in the sand since in this case it is unrelated to
% growth rate.
...
% to the buried biomass percentage. In this case it is a constant value
% (not related to buried biomass percentage).
...
for i = 2:imax %this is the time loop
    ...
    if grass_height(i-1)<=max_grass_height %if the grass is shorter than the max height, apply the following growth laws
            grass_growth(i) = (min_growth*grass_height(i-1)/max_grass_height*(1-grass_height(i-1)/max_grass_height))*dt;%growth curve for grass G = G(1-B) based on biomass
    ...
    range_of_growth_factors(i) = 1; %growth factor is a increasing with additional coverage
