% Grasses on a Sandy Beach: Harmful Relationship with Burial
...
% In this particular version of the model, the plant has a harmful
% relationship with burial, such as that in any plant that has not evolved
% to thrive specifically in dunes. This means that the growth rate of the
% plant is based on the following equation: G = c*gB(1-B), where G is the
% growth rate in vertical height/time, g is a scaling factor of a base
% growth rate, and B is the scaled biomass of the plant - in this case B is
% represented by the plant's height as a percentage of a max possible
% growth height. The "growth factor" term, c, accounts for how much of the
% plant is buried in the sand. The more the plant is buried, the more it
% hurts the growth rate. There is a plant death burial percentage, at which
% point the plant growth rate goes to zero.  Once the growth rate stalls,
% the burial amount continues to increase due to diffusion of the
% accumulated dune. This starts irreversible the death of the plant, where
% the growth rate becomes negative until the whole plant is gone.
...
for i = 2:imax %this is the time loop
    ...
    plant_buried_time = find(plant_area_covered_percent>.99);
    
    %Correct for above ground coverage when the plant dies
    false_growth1 = find(plant_area_covered_percent<=0); %find negative coverage
    false_growth2 = find(plant_area_covered_percent>.99); %find coverage >100%
    plant_area_covered_percent(false_growth1) = 1; %replace with 100% coverage
    plant_area_covered_percent(false_growth2) = 1; %replace with 100% coverage
    
    %Grow the Grass
    if plant_area_covered_percent(i) == 1 %if the plant is totally buried
        grass_growth(i) = 0; %it's not growing
        biomass_growth(i)=0; %no growth
        burial_growth_factor(i) = 0; %no growth factor either
    else
        if grass_height(i-1)<=max_grass_height %if the grass is shorter than the max height, apply the following growth laws

                grass_growth(i) = (min_growth*grass_height(i-1)/max_grass_height*(1-grass_height(i-1)/max_grass_height)*(1-plant_area_covered_percent(i)/plant_death_coverage_percent))*dt;%growth curve for grass G = c*G(1-B) scaled by covered percentage factor, c
                ...
                burial_growth_factor(i) = (1-plant_area_covered_percent(i)); %the real time burial growth factor affecting the plant is the c term, here it hurts the plant with increasing burial
    ...
    range_of_growth_factors(i) = (1-0.01*range_of_percents(i)/plant_death_coverage_percent); %growth factor is a increasing with additional coverage
...
for i = plant_buried_time(3):imax
        grass_growth(i) = 0; %it's not growing
        biomass_growth(i)=0; %no growth
        burial_growth_factor(i) = 0; %no growth factor either
        grass_height(i) = 0;
...
legend('Total Grass Growth per day','Grass Growth without Burial Growth Factor','Location','southwest')
set(gca,'fontsize',16,'fontname','arial')
axis([0 tmax -.5 .5])

%Plant Growth Factors
subplot(3,1,3)
plot(range_of_percents,range_of_growth_factors,'k')
hold all
